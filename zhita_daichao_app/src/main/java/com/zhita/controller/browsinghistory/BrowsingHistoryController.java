...
public class BrowsingHistoryController {
	...
	@ResponseBody
	@Transactional
	public Map<String, Object> insertFootprint(String userId, int curPage, String company, String oneSourceName,
			String twoSourceName) { // userId是用户的id，curPage是页码
		...
		if (StringUtils.isEmpty(userId) || StringUtils.isEmpty(curPage) || StringUtils.isEmpty(company)) {
			...
			if (oneSourceName == null && twoSourceName == null) {
				...
				List<String> list = (List<String>) cFootprintService.getbusinessName(userId, pageSize, startRow,
						company);// 获取商品名称
				...
				if (tableType.equals("2")) {
					...
					long recordNumber = commodityFootprintCopyService.getRecordNumberAppCopy(userId, company,
							oneSourceName, twoSourceName);// 根据用户id和公司名称，查询出他有多少条唯一商品名称的足迹
					int largestPages = (int) (recordNumber / 10) + 1;// 最大页数
					if (largestPages < curPage) {
						curPage = largestPages;
					}
					...
					Map<String, Object> map = new HashMap<String, Object>();
					List<Map<String, Object>> goodslist = new ArrayList<Map<String, Object>>();
					List<String> list = (List<String>) commodityFootprintCopyService.getbusinessNameAppCopy(userId,
							pageSize, startRow, company, oneSourceName, twoSourceName);// 获取商品名称
					for (String businessName : list) {
						map = intRegisteCopyService.getLoansBusinessesAppCopy(businessName, company, oneSourceName,
								twoSourceName); // 获取商品的所有信息
						if (map != null) {
							fakeApplications = (int) map.get("applications"); // 假的申请人数
							String loanlimitbig = ((BigDecimal) map.get("loanlimitbig")).setScale(0) + "";
							String loanlimitsmall = ((BigDecimal) map.get("loanlimitsmall")).setScale(0) + "";
						...
						long applications = commodityFootprintCopyService.getApplicationsAppCopy(businessName, company,
								oneSourceName, twoSourceName) + fakeApplications; // 获取申请人数
						if (map == null) {
							continue;
						}
						map.put("applications", applications);
